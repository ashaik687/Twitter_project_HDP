...
----Here I am Partitioning the tables based on the Sentiment. All the Persons with Positive sentiment are in one partition and
-- all the people with negative are in another partition and another partition will have neutral
		
-- Partitions are applied on tweets_sentiment table look at the code for table tweets_sentiment
...
-- case 
 ...
 --from l3 group by id;
 --partitioned BY ( analysis string );
 --alter table tweets_sentiment add partition (analysis='positive');
--alter table tweets_sentiment add partition (analysis='negative');
--alter table tweets_sentiment add partition (analysis='neutral');
...
ROW FORMAT DELIMITED FIELDS TERMINATED BY '\t' 
STORED AS TEXTFILE
LOCATION '/user/hue/time_zone_map';
